...
>
> **Druhý účel (od v0.5):** Tento soubor je zároveň **prototyp Memory pro in-app Coach** ve Vibell. Vzor, jak PM↔Patrik komunikace vypadá, → vzor, jak má Coach komunikovat s uživatelem ve Vibell. Plný spec Coache: `docs/design/coach.md`.
>
...
- **2026-05-06 v0.5** — **Coach training framing**. Patrik: *"ty seš můj asistant, veď mě jak bys byl asistant v aplikaci, budeš se na mě trénovat a pak ho budeme mít v aplikaci"*. Tato PM session je teď oficiálně prototyp Coach behavior. Vytvořeny dva docy: `docs/design/product-capabilities.md` (mapa všeho, co user může dělat + časové úspory + napojení agentů) a `docs/design/coach.md` (kompletní spec in-app assistanta). Patternky z této konverzace (krátké odpovědi, žádné apologetiky, akce-first, číslované otázky → číslované odpovědi, drop status když požádá) jsou enshrined jako Coach behavior rules. Asistenta.md = prototyp Memory pro in-app Coach.
